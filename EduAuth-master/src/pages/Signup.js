...
const Signup = () => {
  ...
  const uploadFaceImage = (e) => {
    ...
    const faceImageRef = ref(
      storage,
      `face/${timestamp}_${faceImageUpload.name}`
    );
  ...
  return (
    <form className="signup" onSubmit={handleSubmit}>
      ...
      <input
        type="file"
        onChange={(event) => {
          setNicImageUpload(event.target.files[0]);
        }}
      />
      <button style={{ backgroundColor: 'orange' }} onClick={uploadNicImage}>
        {' '}
        Upload NIC{' '}
      </button>
